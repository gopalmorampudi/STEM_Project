...
//insert tutor details----Tutor Registration
	...
	$sql="insert into tutor(tutor_name,instructor_id,emailid,password,phone_number,subject,working_hours,working_days) values('$tutor_name','$instructorID','$emailid','$password','$phone_number','$sub','$working_hours','$working_days')";
	$res=mysql_query($sql);
	print_r($subject);
	...
	if($res){	
		echo "ok";
		echo $sql1="select tutor_id from tutor where emailid='$emailid'";
...
//tutor update profile
	...
	//echo $res;
	if($res){
		 // echo "sucesss";tutor_profile.php?log=sucess
...
$query="insert into student(student_name,emailid,password,major,year,term,stu_id) values('$student_name','$emailid','$password','$major',$ay,'$term','$student_id')";
...
header('Location: login_student.php?mesg=success');
	...
	$_SESSION['flag']='failed';
header('Location: register.php?mesg=fail');
}
}
...
//Update student...
	...
	echo $sql="update student set student_name='$student_name',major='$major',year=$year,term='$term' where stu_id='$student_id'";
 	$res=mysql_query($sql);
	if($res){
		$_SESSION['flag']='success';
...
$message->setTo(array('nukesh.smts@gmail.com'=> $customer_name.''));
...
$transport->setUsername('ticketraisingsystem@gmail.com');

$transport->setPassword('ticket12345');
