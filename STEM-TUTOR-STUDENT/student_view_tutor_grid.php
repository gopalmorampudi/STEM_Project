...
<td style="background:green;color:white;cursor:pointer"  onclick="window.location='appointment_popup.php?id=<?=$a[0]?>&date=<?=$a[3]?>&time=<?=$timings?>&student=<?=$student?>&tutor=<?=$user?>&subject=<?=$sid?>';">
<?php 
				}
				else if($a[11]=='class'){
...
<a href='#' onclick="getNextGrid();" style="float:right;" class="btn">next&gt;</a>
</div>
...
<div class="selectDate" style="display:none;" id="selectForm">
		...
		<span style="font-weight:bold;">Select Date :</span><br><input type="date" name="selectedDate" id="selectedDate">
		<br><br>
		...
		<input type="text" size="3" maxlength="2" name="hours" placeholder="HH"/>
		<span style="font-weight:bold;">:</span>
		<input type="text" size="3" maxlength="2" name="minutes" placeholder="MM"/>
		<select id="<?='fhour'.$i.$j?>" name="period">
